...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  private void initialize(boolean bypassVoiceProcessing, int networkIgnoreMask, boolean forceSWCodec,
    ...
    // 🔥 FORCED RAVE MODE: Override AudioAttributes to MEDIA/MUSIC defaults if not
    // set.
    // This prevents Android from treating the app as a "Phone Call" (VoIP) and
    // triggering unwanted
    // persistent notifications or call recording indicators.
    if (audioAttributes == null) {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        audioAttributes = new AudioAttributes.Builder()
            ...
            .build();
        Log.d(TAG, "Forcing AudioAttributes to USAGE_MEDIA (Rave Mode)");
      }
    }
